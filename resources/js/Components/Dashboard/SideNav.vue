<template>
  <aside class="fixed inset-y-0 flex-wrap items-center justify-between block w-full p-0 my-4 overflow-y-auto antialiased transition-transform duration-200 -translate-x-full bg-white border-0 shadow-xl dark:shadow-none dark:bg-slate-850 max-w-64 ease-nav-brand z-990 xl:ml-6 rounded-2xl xl:left-0 xl:translate-x-0" aria-expanded="false">
      ...
      <div class="items-center block w-auto max-h-screen overflow-auto h-full grow">
        <ul class="flex flex-col pl-0 mb-0">

          <li class="mt-0.5 w-full" v-for="item,idx in menu" :key="idx">
            <a class=" dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors" href="../pages/tables.html">
              ...
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease">{{item.label}}</span>
            </a>
          </li>
